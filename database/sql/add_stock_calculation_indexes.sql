...
-- Index on orders.reference_no for joining with order_items
CREATE INDEX IF NOT EXISTS idx_orders_reference_no ON orders(reference_no);
...
-- Index on order_items.reference_no for joining items to orders (using reference_no)
CREATE INDEX IF NOT EXISTS idx_order_items_reference_no ON order_items(reference_no);
...
-- Composite index on order_items(reference_no, product_no) for efficient join + filter
CREATE INDEX IF NOT EXISTS idx_order_items_ref_product ON order_items(reference_no, product_no);
