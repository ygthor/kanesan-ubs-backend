...
@section('breadcrumbs')
    ...
    <li class="breadcrumb-item"><a
            href="{{ route('inventory.stock-management') }}{{ $agentNo ? '?agent_no=' . urlencode($agentNo) : '' }}">Stock
            Management</a></li>
    <li class="breadcrumb-item active">{{ $item->ITEMNO }} - Transactions</li>
@endsection

@section('card-title', 'Agent No: ' . $agentNo . ' | ' . 'Item No: ' . $item->ITEMNO)

@section('card-tools')
    @if ($agentNo)
        <a href="{{ route('inventory.stock-management.create', ['agent_no' => $agentNo, 'group' => $item->GROUP ?? '', 'item_code' => $item->ITEMNO]) }}"
            class="btn btn-primary btn-sm">
            <i class="fas fa-plus"></i> Add Stock
        </a>
    @endif
    <a href="{{ route('inventory.stock-management') }}{{ $agentNo ? '?agent_no=' . urlencode($agentNo) : '' }}"
...
@section('admin-content')
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form method="GET" action="{{ route('inventory.stock-management.item.transactions', $item->ITEMNO) }}"
                id="filterForm">
                @if ($agentNo)
                ...
                <div class="row mb-3">
                    <div class="col-12">
                        ...
                        <div class="btn-group" role="group">
                            ...
                            <button type="button" class="btn btn-sm btn-outline-primary"
                                onclick="setDateRange('last_3_months')">Last 3 Months</button>
                            <button type="button" class="btn btn-sm btn-outline-primary"
                ...
                <div class="row">
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-control select2" name="transaction_type" id="transactionTypeFilter"
                            style="width: 100%;">
                            <option value="">All Transaction Types</option>
                            <option value="in" {{ request('transaction_type') == 'in' ? 'selected' : '' }}>Stock In
                            </option>
                            <option value="out" {{ request('transaction_type') == 'out' ? 'selected' : '' }}>Stock Out
                            </option>
                            <option value="invoice_sale"
                                {{ request('transaction_type') == 'invoice_sale' ? 'selected' : '' }}>Invoice Sale</option>
                            <option value="invoice_return"
                                {{ request('transaction_type') == 'invoice_return' ? 'selected' : '' }}>Invoice Return
                            </option>
                            ...
                            @forelse($transactions as $trans)
                                @php
                                    ...
                                    $stockBefore = is_array($trans)
                                        ...
                                        : $trans->stock_before ?? null;
                                    $stockAfter = is_array($trans)
                                        ? $trans['stock_after'] ?? null
                                        : $trans->stock_after ?? null;
                                    $referenceId = is_array($trans)
